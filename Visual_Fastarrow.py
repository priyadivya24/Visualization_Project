...
def get_initial_date(main_folders, sub_folders, subsub_folder):
    subsubsubfolder_path = os.path.join(main_folders, sub_folders, subsub_folder)
    files = [f for f in os.listdir(subsubsubfolder_path) if f.endswith('.parquet')]
    if not files:
        return None
    first_file = os.path.join(subsubsubfolder_path, files[0])
    df = fastparquet.ParquetFile(first_file).to_pandas()
    initial_date = df['timestamp'].min()
    return initial_date
...
def update_line_plot(selected_subfolder, selected_date, main_folders, sub_folders, subsub_folder):
...
@app.callback(Output('line-plot-1', 'figure'),
              [Input('subfolder-dropdown', 'value'),
               Input('date', 'date')])
def update_graph_1(selected_subfolder, selected_date):
    fig = update_line_plot(selected_subfolder, selected_date, main_folder, sub_folder, subsub_folder_1)
    return fig
...
@app.callback(Output('line-plot-2', 'figure'),
              [Input('subfolder-dropdown-2', 'value'),
               Input('date', 'date')])
def update_graph_2(selected_subfolder, selected_date):
    fig = update_line_plot(selected_subfolder, selected_date, main_folder, sub_folder, subsub_folder_2)
    return fig
...
@app.callback(Output('line-plot-3', 'figure'),
              [Input('subfolder-dropdown-3', 'value'),
               Input('date', 'date')])
def update_graph_3(selected_subfolder, selected_date):
    fig = update_line_plot(selected_subfolder, selected_date, main_folder, sub_folder2, subsub_folder_3)
    return fig
...
@app.callback(Output('line-plot-4', 'figure'),
              [Input('subfolder-dropdown-4', 'value'),
               Input('date', 'date')])
def update_graph_4(selected_subfolder, selected_date):
    fig = update_line_plot(selected_subfolder, selected_date, main_folder, sub_folder2, subsub_folder_4)
    return fig
