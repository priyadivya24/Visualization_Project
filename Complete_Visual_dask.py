...
def read_parquet_files(subfolder_path):
    ...
    dataframes = [dd.read_parquet(os.path.join(subfolder_path, f)) for f in files]
    return dd.concat(dataframes, axis=0).compute()
...
def downsample_data(df):
    ...
    df_resampled = df_resampled.reset_index().compute()
    return df_resampled
...
def update_line_plot(selected_subfolder, main_folders, sub_folders, subsub_folder):
    ...
    df = convert_timestamp(df).compute()
    df = dd.from_pandas(df, npartitions=1)
    df_resampled = downsample_data(df)
    fig = make_subplots(specs=[[{'secondary_y': True}]])
    fig.add_trace(px.line(df_resampled, x='timestamp', y='data').data[0])
    return fig
...
# Callbacks for plots individually
