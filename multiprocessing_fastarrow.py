...
def read_parquet_file_parallel(file):
    return fastparquet.ParquetFile(file).to_pandas()
...
def read_parquet_files_parallel(subfolder_path):
    ...
    with multiprocessing.Pool() as pool:
        tables = pool.map(read_parquet_file_parallel, files)
...
def get_initial_date_range(main_folders, sub_folders, subsub_folder):
    subsubsubfolder_path = os.path.join(str(main_folders), str(sub_folders), str(subsub_folder))
